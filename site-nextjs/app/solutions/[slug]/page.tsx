...
export function generateMetadata({params}:{params:{slug:string}}):Metadata{const s=sectors.find(x=>x.slug===params.slug);return s?{title:`Installation de bornes électriques – ${s.short}`,description:`Installation de bornes électriques pour ${s.short.toLowerCase()} en Belgique, en Wallonie et dans les Hauts-de-France. ${s.description}`}:{}}
export default function SectorPage({params}:{params:{slug:string}}){const s=sectors.find(x=>x.slug===params.slug);if(!s)notFound();return <>
  ...
  <section className="bg-slate-50 py-24"><div className="container grid items-center gap-12 lg:grid-cols-2"><div><SectionTitle eyebrow="Installation de bornes électriques" title="Le bon niveau de service, du pilote au déploiement transfrontalier"/><p className="mt-6 text-lg leading-8 text-slate-600">{s.offer}</p><p className="mt-4 leading-7 text-slate-600">Nos équipes prennent en charge les travaux en Belgique, en Wallonie et dans les Hauts-de-France, puis sélectionnent la marque de borne adaptée au projet.</p><Link href={`/devis?secteur=${s.slug}`} className="btn-dark mt-8">Étudier mon projet <ArrowRight size={17}/></Link></div><div className="rounded-4xl bg-navy-950 p-9 text-white"><p className="text-sm font-bold uppercase tracking-widest text-electric-400">Exemple de déploiement</p><p className="mt-8 text-3xl font-black leading-tight">{s.caseStudy}</p><p className="mt-6 text-sm text-slate-400">Cas présenté à titre illustratif. Les résultats dépendent du contexte technique et opérationnel.</p></div></div></section>
